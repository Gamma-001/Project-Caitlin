...
module.exports = class extends Command {
   constructor(...args) {
      super(...args, {
         name: 'i-cuddle',
         enabled: true,
         description: 'Give someone a cuddle with a custom made image',
         runIn: ['text', 'group'],
         cooldown: 5
      });
   }
   async run(msg, [...args]) {
      ...
      let cuddle = loadImage('./src/images/i-cuddle.jpeg').then(image => {
      ...
      return Promise.all([avatar1, avatar2, cuddle]).then(promise => {
         ctx.save()
         ctx.rotate(-0.4);
         ctx.drawImage(canvas1, 67, 105, 55, 55);
         ctx.restore();
         ctx.drawImage(canvas2, 82, 20, 50, 50);
         return msg.channel.send(new MessageAttachment(canvas.toBuffer(), 'cuddle.png'));
      });
   }
};